...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'tldr-pages');
}

const REPO = 'https://raw.githubusercontent.com/tldr-pages/tldr/main';
// tldr.sh/assets/index.json was removed circa 2026; the canonical index now
// ships with each tldr release on GitHub as `index.json`. Same JSON shape.
const INDEX = 'https://github.com/tldr-pages/tldr/releases/latest/download/index.json';
const UA = 'pipeworx-mcp-tldr-pages/1.0 (+https://pipeworx.io)';
const TTL_MS = 24 * 60 * 60 * 1000;
interface TldrIndex {
  commands: { name: string; platform: string[]; language: string[]; targets?: { os: string; language: string }[] }[];
}
let INDEX_CACHE: { at: number; data: TldrIndex } | null = null;
...
const tools: McpToolExport['tools'] = [
  ...
  {
    name: 'commands',
    description: 'List every command-line utility that has a tldr-pages cheatsheet, filtered to one platform (common, linux, osx, windows, android, sunos, freebsd) and one ISO 639-1 language, read from the tldr-pages index published on GitHub. Returns the command names plus a count. Answers which shell commands tldr documents for a given platform, before fetching a page.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  switch (name) {
    case 'page': {
      ...
      if (res.status === 404) {
        // Fall back to common.
        if (platform !== 'common') {
          const fb = await pwFetch(`${REPO}/${dir}/common/${cmd}.md`, { headers: { 'User-Agent': UA } });
      ...
      if (!res.ok) throw await httpError(res, 'tldr');
...
async function loadIndex() {
  ...
  const res = await pwFetch(INDEX, { headers: { Accept: 'application/json', 'User-Agent': UA } });
  if (!res.ok) throw await httpError(res, 'tldr index');
  const data = (await res.json()) as TldrIndex;
